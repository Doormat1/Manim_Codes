...
# Fixed ceiling support
        ...
        with self.voiceover(text="""
            Now lets look at a 2nd example of SHM in practice, a mass attached to a spring. 
            If we displace the mass beyond the equilibrium point, and let it go, it will oscillate up and down around this point. 
            In a real example, the oscillations decrease in amplitude over time due to damping forces. However for our ideal example, we can ignore these. <bookmark mark="spring"/>
            Lets stretch and hold our spring and look at the forces involved. We have weight acting downwards, equal to m g and the spring force, equal to spring constant, k times the displacement, labelled x here. <bookmark mark="forces_spring"/>
            The natural length is the length of our spring if it is unstretched, it is from this length that we measure all the extensions.
            The equilibrium position is where these forces would be balance and the resultant force would be zero. <bookmark mark="derivationstart"/>
            Ok lets start with the derivation, the force on a spring is equal to the spring constant times the extension. <bookmark mark="one"/>
            At the equilibrium position, the forces are balanced, so weight minus the spring costant would equal zero, so the weight is equal to k times the extension the weight causes, x in this setup. <bookmark mark="two"/>
            Now if we look at the forces in our extra stretched position. Here the weight is the same but the spring force now is increased, as the extension has, so the resultant force F, now equals m g - k(x+y) <bookmark mark ="three"/>
            If we expand our brackets, we get that the resultant force equals, mass times g minus k x minus ky. <bookmark mark="four"/>
            Earlier we established that k x equals m g, so they cancel out leaving us with F = minus k y. <bookmark mark="five"/>
            Right now lets tidy up <bookmark mark="tidy"/>
            Lets look at the SHM side of things, applying our condition for SHM, we get that y equals minus omega squared a. <bookmark mark="six"/>
            Again we apply Newton's 2nd law and sub this in to get force equals minus m y divided by omega squared. <bookmark mark="seven"/>
            Again a quick tidy. 
            As we have two formulas for the same force, we can make them equal to each other <bookmark mark="eight"/>
            Now we can cancel to get one over omega squared = k over m. <bookmark mark="nine"/>          
            Finally we can substitue our defintion for omega in, to get our final formula for a spring in SHM, t equals 2 pi times the square root of m over k. 
            """)as tracker:
            # Add initial objects
            ...
            self.wait_until_bookmark("spring")
            self.play(
            ...
            self.wait_until_bookmark("forces_spring")
            self.wait(2)
            ...
            self.wait_until_bookmark("derivationstart")
        ...
        Summary_title_underline = Underline(Summary_title)
        Summary = VGroup(
            Summary_title,
            Summary_title_underline,
            MathTex(r"x = A sin\ \omega t "),
            MathTex(r"x \propto -a"),
            MathTex(r"T_{pendulum} = 2\pi\ \sqrt{\frac{l}{g}}"),
            MathTex(r"T_{spring} = 2\pi\ \sqrt{\frac{m}{k}}")
        ).arrange(DOWN, buff=0.4).to_edge(UP)
        with self.voiceover(text = """
            So in summary we have shown that x = A sin omega t is a valid equation for an object in shm.
            We used the conditon for shm, displacement is proportional to negative acceleration to derive two formulas,
            For a pendulum, period = 2 pi times the square root of length over g.
            And for a spring, period equals 2 pi times the square root of the mass divided by the spring constant.
            The methodology used here is applicable for any system in SHM, look for the forces involved, try to get a formula where force is proportional to negative displacement,
            before applying the condition for SHM to get a final formula. 
                            
            Hope this all helps and thanks for watching.

            """)as tracker:
            self.play(
                LaggedStart(
                    *[Write(item) for item in Summary],
                    lag_ratio=0.8
                )
            )
        self.wait(5)
